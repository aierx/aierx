...
dberr_t row_search_mvcc(byte *buf, page_cur_mode_t mode,
  ...
  if (UNIV_UNLIKELY(direction == 0)) {
    ...
    if (UNIV_UNLIKELY(direction != prebuilt->fetch_direction)) { // 如果查询方向改变，则说明之前的缓存没有用了，清空即可
    ...
    } else if (UNIV_LIKELY(prebuilt->n_fetch_cached > 0)) {  // 方向相同，且缓存中还有未读取的行，直接使用缓存数据即可
...
rec_loop:
  ...
  if (prebuilt->select_lock_type != LOCK_NONE) { // 当前读模式
    ...
    if (row_to_range_relation.row_can_be_in_range) {
      ...
      if (row_to_range_relation.gap_can_intersect_range) {
        ...
        err = DB_RECORD_NOT_FOUND;
        goto normal_return; // 4、如果都不在索引范围内，未找到
      }
    }
  ...
  locks_ok:
    ...
    if (trx->isolation_level == TRX_ISO_READ_UNCOMMITTED) {
      ...
      if (srv_force_recovery < 5 &&
        ...
        rec_t *old_vers; /* 如果当前行不可见，使用undo log构建历史版本，直到可见版本 */
        /* The following call returns 'offsets' associated with 'old_vers' */
        err = row_sel_build_prev_vers_for_mysql(
      ...
      if (!srv_read_only_mode &&
        ...
        index entry. */ /* 如果不可见，需要先进行索引下推判断，然后进行回表操作 */
        switch (row_search_idx_cond_check(buf, prebuilt, rec, offsets)) {
          case ICP_NO_MATCH:
            goto next_rec;
          case ICP_OUT_OF_RANGE:
  ...
  if (rec_get_deleted_flag(rec, comp)) { // 记录存在删除标记
    /* The record is delete-marked: we can skip it */
    ...
    if the same record is deleted twice by the same transaction */ /* 如果是唯一索引，则退出 */
  ...
  requires_clust_rec:
    ...
    built for a consistent read. */   /* 通过二级索引回表获取聚集索引记录，注意此时已经讲旧版本也读取了. */
...
static const rec_t *row_search_get_max_rec(dict_index_t *index, mtr_t *mtr) {
  btr_pcur_t pcur;
  const rec_t *rec;
  // 如果是全表扫描或者全索引扫描，将cursor放置B+Tree的一端
